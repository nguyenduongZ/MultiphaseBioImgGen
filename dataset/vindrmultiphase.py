...
class VinDrMultiphase(BaseDataset):
    def __init__(self, opt: Opt):
        ...
        initialize_t5(idx=opt.config["trainer"]["idx"])
    ...
    def _load_data_(self):
        csv_file = self.opt.config["data"][self.dataset_name]["PATH_METADATA_PROMPT_FILE"]
    ...
    def _set_df_splits_(self):
        valid_size = self.opt.config["data"][self.dataset_name]["preprocessing"]["valid_size"]
        random_state = self.opt.config["seed"]
    ...
    def _set_text_embeddings_(self):
        ...
        batch_size = self.opt.config["trainer"]["batch_size"]        
        ...
        for split, df in zip(["train", "valid"], [self.df_train, self.df_valid]):
            path_t5_embedding_file = self.opt.config["data"][self.dataset_name][f"PATH_{split.upper()}_EMBEDDING_FILE"]
    ...
    def get_sample_ds(self):
        ...
        path_sample_embedding_file = self.opt.config["data"][self.dataset_name]["PATH_TEST_EMBEDDING_FILE"]
